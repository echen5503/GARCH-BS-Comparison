...
class GARCHModel:
    ...
    def _sim_terminal(self, S0, T_days, r_daily, h0, n_paths, seed):
        ...
        for _ in range(T_days):
            ...
            # Without normal MGF, pure exponentiation drifts S upward unfairly
            S *= np.exp(r_daily - 0.5*h + np.sqrt(h)*z)
            h = ω + α*z**2*h + β*h
            np.clip(h, 1e-12, 2.0, out=h)
            
        # FIX 1: Empirical Martingale Simulation (EMS) forces Risk-Neutrality
...
class Researcher:
    ...
    def price_options(self, opts: pd.DataFrame) -> pd.DataFrame:
        ...
        # --- OUTLIER REMOVAL FIX ---
        # Filter out the top and bottom 2% of extreme pricing errors 
        # (removes stale/illiquid quotes that ruin the graph scale)
        if len(opts) > 20:
            q_low = opts["bs_error"].quantile(0.02)
            q_high = opts["bs_error"].quantile(0.98)
            opts = opts[(opts["bs_error"] >= q_low) & (opts["bs_error"] <= q_high)]
